...
describe('GET /v1/movies/popular', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    (movieService.fetchMoviePage as jest.Mock).mockResolvedValue(mockPopularMovieResults);
  });
  ...
  it('respects the limit parameter', async () => {
    ...
    const res = await request(app).get('/v1/movies/popular?limit=3');
    expect(res.status).toBe(200);
    expect(res.body.length).toBeLessThanOrEqual(3);
  });

  it('returns 400 when limit is not a number', async () => {
    const res = await request(app).get('/v1/movies/popular?limit=abc');
  ...
  it('returns 400 when limit exceeds maximum', async () => {
    const res = await request(app).get('/v1/movies/popular?limit=999');
    expect(res.status).toBe(400);
    expect(res.body).toHaveProperty('error');
