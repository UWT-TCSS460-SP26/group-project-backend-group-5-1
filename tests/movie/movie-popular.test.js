...
describe('GET /v1/movies/popular', () => {
    beforeEach(() => {
        jest.clearAllMocks();
        movieService.fetchMoviePage.mockResolvedValue(mockPopularMovieResults);
    });
    it('returns 200 and correct fields on success', async () => {
        const res = await (0, supertest_1.default)(app_1.app).get('/v1/movies/popular');
    ...
    it('strips fields not in the allowlist', async () => {
        ...
        expect(res.body[0]).not.toHaveProperty('overview');
    });
    it('respects the limit parameter', async () => {
        const manyResults = Array.from({ length: 20 }, (_, i) => ({
            ...mockPopularMovieResults[0],
            id: i + 1,
            title: `Movie ${i + 1}`,
        }));
        movieService.fetchMoviePage.mockResolvedValue(manyResults);
        const res = await (0, supertest_1.default)(app_1.app).get('/v1/movies/popular?limit=3');
        expect(res.status).toBe(200);
        expect(res.body.length).toBeLessThanOrEqual(3);
    });
    it('returns 400 when limit is not a number', async () => {
        const res = await (0, supertest_1.default)(app_1.app).get('/v1/movies/popular?limit=abc');
        expect(res.status).toBe(400);
        expect(res.body).toHaveProperty('error');
        expect(res.body.error).toBe('limit must be a positive integer');
    });
    it('returns 400 when limit exceeds maximum', async () => {
        const res = await (0, supertest_1.default)(app_1.app).get('/v1/movies/popular?limit=999');
        expect(res.status).toBe(400);
        expect(res.body).toHaveProperty('error');
...
//# sourceMappingURL=movie-popular.test.js.map