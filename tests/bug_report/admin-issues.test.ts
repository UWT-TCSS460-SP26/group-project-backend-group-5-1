...
describe('GET /v1/issues', () => {
  it('returns list of issues for admin', async () => {
    (prisma.$queryRaw as jest.Mock).mockResolvedValueOnce([fakeIssue]);
...
describe('PATCH /v1/issues/:id', () => {
  it('updates status to InProgress', async () => {
    (prisma.$queryRaw as jest.Mock).mockResolvedValueOnce([fakeIssue]);
    (prisma.$queryRaw as jest.Mock).mockResolvedValueOnce([{ ...fakeIssue, status: 'InProgress' }]);
  ...
  it('updates status to Resolved', async () => {
    (prisma.$queryRaw as jest.Mock).mockResolvedValueOnce([fakeIssue]);
    (prisma.$queryRaw as jest.Mock).mockResolvedValueOnce([{ ...fakeIssue, status: 'Resolved' }]);
    const res = await request(app)
      .patch('/v1/issues/1')
      .set(asAdmin)
  ...
  it('does not update a different issue', async () => {
    (prisma.$queryRaw as jest.Mock).mockResolvedValueOnce([fakeIssue2]);
    (prisma.$queryRaw as jest.Mock).mockResolvedValueOnce([{ ...fakeIssue2, status: 'Resolved' }]);
    const res = await request(app)
      .patch('/v1/issues/2')
      .set(asAdmin)
      .send({ status: 'Resolved' });
